...
class Sensor(SensorProtocol, Device):
    def __init__(self, device_id: int, device_service: DeviceServiceProtocol, io_service: IOServiceProtocol, event_dispatcher: EventDispatcherProtocol, config: SensorConfigDto, ai_id: int):
        ...
        self.__alarm_manager = SensorAlarManager(current_config)
    ...
    def value_scaled_max(self) -> float:
        return self.__alarm_manager.config.value_scaled_max
    @value_scaled_max.setter
    def value_scaled_max(self, value: float) -> None:
        self.__alarm_manager.config.value_scaled_max = value
        self.device_service.set_sensor_config(self.device_name, self.__alarm_manager.config)

    @property
    def value_scaled_min(self) -> float:
        return self.__alarm_manager.config.value_scaled_min
    @value_scaled_min.setter
    def value_scaled_min(self, value: float) -> None:
        self.__alarm_manager.config.value_scaled_min = value
        self.device_service.set_sensor_config(self.device_name, self.__alarm_manager.config)
    ...
    @property
    def ai_max(self) -> int:
        return self.__alarm_manager.config.ai_max
    @ai_max.setter
    def ai_max(self, value: int) -> None:
        self.__alarm_manager.config.value_ai_max = value
        self.device_service.set_sensor_config(self.device_name, self.__alarm_manager.config)

    @property
    def ai_min(self) -> int:
        return self.__alarm_manager.config.ai_min
    @ai_min.setter
    def ai_min(self, value: int) -> None:
        self.__alarm_manager.config.value_ai_min = value
        self.device_service.set_sensor_config(self.device_name, self.__alarm_manager.config)
    ...
    def to_serializable(self) -> SerializableProtocol:
        return SensorDto(
            ...
            is_high_high_critical=self.__alarm_manager.config.is_high_high_critical,
            is_low_low_critical=self.__alarm_manager.config.is_low_low_critical,

            value_scaled=round(self.value_scaled, 1),
            value_ai=self.ai,
            value_ma=round(self.value_ma, 1),
            is_high_active=self.__alarm_manager.is_high_active,
            is_high_high_active=self.__alarm_manager.is_high_high_active,
            is_low_active=self.__alarm_manager.is_low_active,
            is_low_low_active=self.__alarm_manager.is_low_low_active,
        )
